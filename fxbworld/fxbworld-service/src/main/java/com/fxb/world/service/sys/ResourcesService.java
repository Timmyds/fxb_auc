...
public class ResourcesService  {
   ...
   /**
	...
	public Result findResListByProSerivce(Result result, String proId) throws Exception {
		List<TResources> srpList = (List<TResources>) SerializeUtil.unserialize(RedisSlave.getInstance().get("REDIS_KEY_ALL_MUEN_VALUE"));
		if (srpList == null || srpList.size() == 0) {
			 srpList = resourcesMapper.selectAll();
			
			RedisSlave.getInstance().setObject("REDIS_KEY_ALL_MUEN_VALUE", SerializeUtil.serialize(srpList), 5000);
		} else {
			RedisSlave.getInstance().expire("REDIS_KEY_ALL_MUEN_VALUE", 5000);
		...
		for (TResources sysResourcesPo : srpList) {
			...
			BeanUtils.copyProperties(loginRm, sysResourcesPo);
